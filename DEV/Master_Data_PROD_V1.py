...
def read_csv(table_name,_delimeter):
    ...
    df = spark.read.option("recursiveFileLookup", "true").format("csv").load(file_path, inferSchema=False, header = True, delimiter = _delimeter , multiLine = True)
...
#display(df_cust_2.filter(col("customer_code")=="105345").distinct())
...
sales_df_24 = sales_df_23_1.withColumn("MTD", round(col("MTD"),2)).withColumn("MTD_MT", round(col("MTD_MT"),2)).withColumn("MTD_CB", round(col("MTD_CB"),2)).withColumn("LY_end_date", add_months(col('new_end_date'),-12))\
...
base_data_lymtd = sales_df_24.select('exception_key', 'billing_date', 'MTD',"MTD_CB","MTD_MT")
lymtd_base_final = lymtd_base.join(base_data_lymtd,[lymtd_base.exception_key==base_data_lymtd.exception_key,(base_data_lymtd.billing_date>=lymtd_base.LY_first_date_month)&(base_data_lymtd.billing_date<=lymtd_base.LY_end_date)],"inner").select(lymtd_base["*"],base_data_lymtd["MTD"].alias("LYMTD"),base_data_lymtd["MTD_MT"].alias("LYMTD_MT"),base_data_lymtd["MTD_CB"].alias("LYMTD_CB"))
lymtd_base_final = lymtd_base_final.withColumn("lymtd_month_year",date_format(col("LY_first_date_month"),"MMM-yy")).select("exception_key","lymtd_month_year","LYMTD","LYMTD_MT","LYMTD_CB").dropDuplicates()

# COMMAND ----------

#w_lymtd= Window.partitionBy('exception_key', 'LY_end_date', 'LY_first_date_month')
...
sales_df_25 = sales_df_25.fillna(0,["LYMTD","LYMTD_CB","LYMTD_MT"]).withColumn("LYMTD", round(col("LYMTD"),2))\
    ...
    .withColumn("L1MTD_first_date_month", add_months(col('first_date_month'),-1)).withColumn("l1mtd_month_year",date_format(col("L1MTD_first_date_month"),"MMM-yy")).withColumn("L2MTD_end_date",when(date_format(col("billing_Date"),"MMM-yy")==date_format(col("current_date"),"MMM-yy"),col("current_date")).otherwise(last_day(add_months(F.col('new_end_date'),-2))))\
...
base_data_bill_data = sales_df_25.select('exception_key', 'billing_date', 'billing_amount','qty_in_kg','billed_quantity','billing_document','billing_item','profit_center').dropDuplicates().persist()
...
# # L2MTD old logic
# l2mtd_base = sales_df_25.select('exception_key', 'L2MTD_end_date', 'L2MTD_first_date_month')
# base_data_l2mtd = sales_df_25.select('exception_key', 'billing_date', 'billing_amount','billing_document','billing_item','profit_center')
# l2mtd_base_final = l2mtd_base.join(base_data_l2mtd,[l2mtd_base.exception_key==base_data_l2mtd.exception_key,(base_data_l2mtd.billing_date>=l2mtd_base.L2MTD_first_date_month)&(base_data_l2mtd.billing_date<=l2mtd_base.L2MTD_end_date)],"inner").select(base_data_l2mtd["*"],l2mtd_base.L2MTD_end_date, l2mtd_base.L2MTD_first_date_month).dropDuplicates()
# l2mtd_base_final = l2mtd_base_final.withColumn("L2MTD",sum(col("billing_amount")).over(Window.partitionBy('exception_key', 'L2MTD_end_date', 'L2MTD_first_date_month')))
# l2mtd_base_final = l2mtd_base_final.select('exception_key', 'L2MTD_end_date', 'L2MTD_first_date_month','L2MTD').dropDuplicates()

# COMMAND ----------

#display(l2mtd_base_final)
...
columns_final = ['customer_group', 'customer_code', 'material_code','exception_key', 'item_category', 'material_description', 'UoM', 'product_category', 'product_brand', 'brand_flavour', 'mrp', 'pack_size', 'pack_format', 'mrp_cb', 'no_of_bisc_pk', 'secondary_pack_qty', 'secondary_pkt_type', 'material_group', 'business_segment' , 'customer_name', 'region', 'state_code', 'district', 'zone', 'zsm_emp_code', 'zsm', 'zsm_email', 'rsm_emp_code', 'rsm',
 ...
 'L2MMTD_MT', 'growth__percentage_MTD_Vs_L2MMTD_MT', 'MTD_per_day_MT','current_date','LY_end_date', 'LY_first_date_month', 'L1MTD_first_date_month','L1MTD_end_date','L2MTD_first_date_month', 'L2MTD_end_date','LM_first_date_month' , 'LM_end_date','LMTD_first_date_month' ,'LMTD_end_date','L2M_AVG_first_date_month' ,'L2M_AVG_end_date', 'current_fy_start', 'current_fy_end', 'last_fy_start', 'last_fy_end', 'YTD', 'YTD_MT', 'YTD_CB', 'LYTD', 'LYTD_MT', 'LYTD_CB', 'day_lapsed','growth_percentage_YTD_vs_LYTD','growth_percentage_YTD_Vs_LYTD_CB','growth__percentage_YTD_Vs_LYTD_MT']
...
filtered_df = sales_df_36.filter(
    to_date(col('billing_month'), 'yyyy-MM-dd').between(
        to_date(lit('2023-01-01'), 'yyyy-MM-dd'), 
